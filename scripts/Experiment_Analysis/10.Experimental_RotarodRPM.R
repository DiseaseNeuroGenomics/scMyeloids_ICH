...
long_data$Timepoint <- factor(long_data$Timepoint,
                              levels = c("RotarodRPMMedians_BSL", "RotarodRPMMedians_Day3", "RotarodRPMMedians_Day7"),
                              ordered = TRUE)
...
gaus_model <- lmer(RotarodRPMMedians_Score ~ Timepoint * Condition + Dose + (1 | Animal.Code), data = long_data)
gamma_model <- glmer(RotarodRPMMedians_Score ~ Timepoint * Condition + Dose + (1|Animal.Code),
...
ggplot(long_data, aes(x = Timepoint, y = RotarodRPMMedians_Score, color = Condition, group = Condition)) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(fun = mean, geom = "point", size = 2) +
  facet_wrap(~ Gender) +
  labs(title = "RotarodRPMMedians Score Over Time by Condition and Gender",
       x = "Timepoint",
