...
N_CTRL = 100                    # matches scdrs.score_cell(n_ctrl=100) in 8.scDRS.py
MCP_FLOOR = 1.0 / (N_CTRL + 1)  # smallest attainable Monte-Carlo p-value
...
def main():
    ...
    # With n_ctrl=100 the smallest attainable assoc_mcp is 1/101 = 0.0099.
    # Values at the floor are bounds, not point estimates, and must be
    # reported as "< 0.01" rather than as an exact p-value.
    d["assoc_mcp_at_floor"] = np.isclose(d["assoc_mcp"], MCP_FLOOR, atol=1e-6)
    d["assoc_mcp_reported"] = np.where(
        d["assoc_mcp_at_floor"], f"<{MCP_FLOOR:.4f}",
        d["assoc_mcp"].map(lambda x: f"{x:.4g}")
    )
    ...
    readme = pd.DataFrame({"Field": [
        ...
        "scdrs.score_cell(ctrl_match_key='mean_var', n_ctrl=100, weight_opt='vs'); "
        "gene sets from MAGMA top-1000 genes per trait.",
        str(N_CTRL),
        ...
        f"{MCP_FLOOR:.4f}. Rows flagged assoc_mcp_at_floor=TRUE are upper bounds and "
        "are reported as '<0.0099', not as exact p-values.",
        ...
        f"Benjamini-Hochberg correction is not informative at n_ctrl={N_CTRL}: because "
        f"the p-value floor is {MCP_FLOOR:.4f}, a BH q-value below 0.05 across a panel "
        "of k tests is only attainable if at least k/5 tests simultaneously sit at the "
        "floor, so the procedure is bounded by the resolution of the permutation rather "
        "than by the data. scDRS group-level associations are therefore reported as "
        "exploratory and are interpreted alongside the effect direction (assoc_mcz), "
