...
init_natmed_fonts <- function() {
  ...
  font_dir <- Sys.getenv("ICH_FONT_DIR")
  if (!nzchar(font_dir)) {
    stop("Set ICH_FONT_DIR to the directory containing the Nimbus Sans font files.")
  }
  sysfonts::font_add(family = "Helvetica",
                      regular = file.path(font_dir, "NimbusSans-Regular.otf"),
                      bold    = file.path(font_dir, "NimbusSans-Bold.otf"),
                      italic  = file.path(font_dir, "NimbusSans-Italic.otf"))
