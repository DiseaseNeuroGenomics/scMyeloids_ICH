...
BASE_DIR     <- Sys.getenv("ICH_PROJECT_DIR")
if (!nzchar(BASE_DIR)) {
  stop("Set ICH_PROJECT_DIR to the project data directory before running figure scripts.")
}
...
GMT_DIR      <- Sys.getenv("ICH_GMT_DIR")
if (!nzchar(GMT_DIR)) {
  stop("Set ICH_GMT_DIR to the MSigDB GMT directory before running enrichment scripts.")
}
